...
public class RedstoneScenes {
	...
	public static void redstoneLink(SceneBuilder scene, SceneBuildingUtil util) {
		...
		scene.world().modifyBlockEntityNBT(link1Select, RedstoneLinkBlockEntity.class,
			nbt -> nbt.put("FrequencyLast", ItemHelper.saveOptional(iron, scene.world().getHolderLookupProvider())));
		scene.idle(7);
		scene.world().modifyBlockEntityNBT(link1Select, RedstoneLinkBlockEntity.class,
			nbt -> nbt.put("FrequencyFirst", ItemHelper.saveOptional(sapling, scene.world().getHolderLookupProvider())));
		scene.idle(20);
		...
		scene.world().modifyBlockEntityNBT(link3Select, RedstoneLinkBlockEntity.class,
			nbt -> nbt.put("FrequencyLast", ItemHelper.saveOptional(gold, scene.world().getHolderLookupProvider())));
		scene.idle(7);
		scene.world().modifyBlockEntityNBT(link3Select, RedstoneLinkBlockEntity.class,
			nbt -> nbt.put("FrequencyFirst", ItemHelper.saveOptional(sapling, scene.world().getHolderLookupProvider())));
		scene.idle(20);
