...
/**
 * Copying and pasting a machine's settings with a clipboard.
 * <p>
 * An {@code @EventBusSubscriber}, so a dedicated server loads it while constructing the mod. The
 * outline it draws around a copyable block and the hover tip it shows are in
 * {@link ClipboardValueSettingsHandlerClient} for that reason -- their bodies name client classes,
 * which the server resolves when it verifies this one.
 */
@EventBusSubscriber
