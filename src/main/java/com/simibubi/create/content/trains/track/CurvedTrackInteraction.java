...
public class CurvedTrackInteraction {
	...
	public static void clientTick() {
		...
		if (mc.options.keyAttack.isDown() && result != null) {
			...
			if (breakProgress >= 1) {
				NetworkHelper.INSTANCE.sendToServer(new CurvedTrackDestroyPacket(breakPos, result.loc()
	...
	public static boolean onClickInput(InputEvent.InteractionKeyMappingTriggered event) {
		...
		if (event.isUseItem()) {
			...
			if (AllItems.WRENCH.isIn(heldItem) && player.isShiftKeyDown()) {
				NetworkHelper.INSTANCE.sendToServer(new CurvedTrackDestroyPacket(result.blockEntity()
						.getBlockPos(),
						result.loc()
							.curveTarget(),
