...
public class ProcessingRecipeParams {
	...
	// 26.2 binds a fluid's components when world data loads, which is after recipes are read, so a
	// result names its fluid through a template and the stack is built from it on demand.
	...
	protected static <P extends ProcessingRecipeParams> MapCodec<P> codec(Supplier<P> factory) {
		return RecordCodecBuilder.mapCodec(instance -> instance.group(
			...
			Codec.either(FluidStackTemplate.CODEC, ProcessingOutput.CODEC).listOf().fieldOf("results")
	...
	protected final List<Either<FluidStackTemplate, ProcessingOutput>> results() {
		List<Either<FluidStackTemplate, ProcessingOutput>> results =
	...
	protected void encode(RegistryFriendlyByteBuf buffer) {
		...
		CatnipStreamCodecBuilders.nonNullList(FluidStackTemplate.STREAM_CODEC).encode(buffer, fluidResults);
	...
	protected void decode(RegistryFriendlyByteBuf buffer) {
		...
		fluidResults = CatnipStreamCodecBuilders.nonNullList(FluidStackTemplate.STREAM_CODEC).decode(buffer);
